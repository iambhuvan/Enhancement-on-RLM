...
def parse_tool_entry(name: str, entry: Any) -> ToolInfo:
    ...
    3. Tuple with description: {"name": (callable_or_value, "description")}

    Args:
        name: The tool name.
        entry: The tool entry (value, dict, or tuple).

    Returns:
        ToolInfo with parsed components.
    """
    # Format 2: Dict with description
    ...
    # Format 3: Tuple with description (used by ERLM)
    if isinstance(entry, tuple) and len(entry) == 2 and isinstance(entry[1], str):
        return ToolInfo(name=name, value=entry[0], description=entry[1])

    # Format 1: No description - treat as plain value
...
def parse_custom_tools(custom_tools: dict[str, Any] | None) -> list[ToolInfo]:
    ...
    Args:
        custom_tools: Dictionary of tool names to values, dicts, or tuples.
