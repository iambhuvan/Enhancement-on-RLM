"""
run_gemini20_flash.py — LongBench-v2 CodeQA eval using Gemini 2.5 Flash Lite
==============================================================================
Evaluates four methods on LongBench-v2 CodeQA samples using Gemini 2.5 Flash Lite
via Vertex AI (Application Default Credentials — no API key needed).
NOTE: Originally targeted gemini-2.0-flash-001 but that model is not accessible
in project newproject-490100. Using gemini-2.5-flash-lite as equivalent (fast,
lightweight Gemini model with same 1M token context window).

Gemini 2.5 Flash Lite context window: 1M tokens ≈ 4M chars.
For docs ≤ 4M chars the base_model can fit the full document without truncation.
RLM adds structure/exploration even when the model can fit the doc in context.
...
Usage
-----
    python run_gemini20_flash.py --n 75
    python run_gemini20_flash.py --n 5 --methods base_model rlm_baseline  # quick test
"""
...
# ---------------------------------------------------------------------------
# Model constants — Gemini 2.0 Flash
# ---------------------------------------------------------------------------

_MODEL_NAME       = "gemini-2.5-flash-lite"   # gemini-2.0-flash not accessible in this project
_MODEL_LABEL      = "gemini25_flash_lite"
...
# RLM loop settings — Gemini API is very fast (~1-3s per call)
_MAX_ITERATIONS = 3
_MAX_TIMEOUT_S  = 300.0   # 5 min; 3 Gemini calls at 2-5s each is well within this
...
class _BaseModelWrapper:
    def __init__(self, backend_kwargs: dict[str, Any]) -> None:
        ...
        self.client = get_client("gemini_vertex", backend_kwargs)
...
def build_rlm_baseline(bkw: dict[str, Any]) -> Any:
    ...
    _iters = [0]
    m = RLM(
        backend="gemini_vertex", backend_kwargs=bkw,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"CodeQA eval — {_MODEL_NAME}"
    )
    ...
    log.info(f"Backend        : Vertex AI / gemini_vertex (project={_VERTEX_PROJECT}, location={_VERTEX_LOCATION})")
    log.info(f"Context window : 1M tokens (~4M chars) — NO thinking mode")
