"""
run_gemini25_flash.py — LongBench-v2 CodeQA eval using Gemini 2.5 Flash Preview
==================================================================================
Evaluates four methods on LongBench-v2 CodeQA samples using Gemini 2.5 Flash
via Vertex AI (Application Default Credentials — no API key needed).
Thinking mode is suppressed via thinking_budget=0 in GenerateContentConfig.

Gemini 2.5 Flash context window: 1M tokens ≈ 4M chars.
Gemini 2.5 Flash is more capable than 2.0 Flash with the same fast inference speed.

Methods
-------
  base_model     — Direct Gemini call, doc truncated to 3.8M chars
...
Usage
-----
    python run_gemini25_flash.py --n 75
    python run_gemini25_flash.py --n 5 --methods base_model rlm_baseline  # quick test
"""
...
# ---------------------------------------------------------------------------
# Model constants — Gemini 2.5 Flash (thinking disabled)
# ---------------------------------------------------------------------------

_MODEL_NAME       = "gemini-2.5-flash"   # Vertex AI model ID (unversioned alias — accessible)
_MODEL_LABEL      = "gemini25_flash"

# Vertex AI settings (Application Default Credentials)
_VERTEX_PROJECT  = "newproject-490100"
_VERTEX_LOCATION = "us-central1"

# Context: 1M tokens; at ~4 chars/token → 4M chars; leave headroom for prompt overhead
_BASE_MODEL_DOC_CHARS = 3_800_000

# RLM loop settings — Gemini 2.5 Flash is fast even without thinking
_MAX_ITERATIONS = 3
_MAX_TIMEOUT_S  = 360.0   # 6 min; slightly more than 2.0 Flash as 2.5 may do more reasoning

# CodeQA sample selection — same 50 docs across all 3 cloud models (seed=42)
# 800K > Claude's 720K cap → consistent cross-model comparison; 4M cap excludes outliers
...
# ---------------------------------------------------------------------------
# Client factory — uses Gemini25VertexClient (Vertex AI, thinking_budget=0)
# ---------------------------------------------------------------------------

def _make_backend_kwargs() -> dict[str, Any]:
    return {
...
class _BaseModelWrapper:
    def __init__(self, backend_kwargs: dict[str, Any]) -> None:
        ...
        self.client = get_client("gemini25_vertex", backend_kwargs)
    ...
    def completion(self, document: str, question: str) -> Any:
        ...
        return RLMChatCompletion(
            root_model=_MODEL_NAME, prompt=prompt, response=response_text,
...
def build_rlm_baseline(bkw: dict[str, Any]) -> Any:
    ...
    _iters = [0]
    m = RLM(
        backend="gemini25_vertex", backend_kwargs=bkw,
...
def build_erlm(bkw: dict[str, Any], o1: bool, o2: bool, o3: bool) -> Any:
    ...
    m = EnhancedRLM(
        backend="gemini25_vertex", backend_kwargs=bkw,
        max_depth=1, max_iterations=_MAX_ITERATIONS, max_timeout=_MAX_TIMEOUT_S,
        max_tokens=20_000_000,      # O2 budget: client may count chars not tokens; 4M char doc × 5 overhead = 20M
...
def run_sample(
    ...
    _RETRY_DELAYS = [30, 60, 90]   # seconds to wait on successive 429/503s

    def _call_with_retry(model):
        for attempt, delay in enumerate([0] + _RETRY_DELAYS, start=0):
            if delay > 0:
                log.warning(f"  [rate-limit] waiting {delay}s before retry {attempt}/{len(_RETRY_DELAYS)}")
                time.sleep(delay)
            try:
                if isinstance(model, _BaseModelWrapper):
                    return model.completion(document, question)
                else:
                    _mc_hint = "\n\nIMPORTANT: Your final answer must be a single letter only: A, B, C, or D."
                    return model.completion(document, root_prompt=question + _mc_hint)
...
def _print_summary(results: list[SampleResult], methods: list[str]) -> None:
    ...
    log.info("\n" + "="*120)
    log.info(f"SUMMARY — {_MODEL_NAME} (Vertex AI, thinking_budget=0) | CodeQA / LongBench-v2")
    log.info("="*120)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"CodeQA eval — {_MODEL_NAME} (thinking disabled)"
    )
    parser.add_argument("--n",       type=int, default=50,
                        help="Number of CodeQA samples (default: 75)")
    ...
    log.info(f"Backend        : Vertex AI / gemini25_vertex (project={_VERTEX_PROJECT}, location={_VERTEX_LOCATION})")
    log.info(f"Context window : 1M tokens (~4M chars)")
    log.info(f"Thinking       : DISABLED (thinking_budget=0)")
